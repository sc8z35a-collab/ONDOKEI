...
同じ1分内の測定はメモリ上の最新値で置き換え、SQLiteには各分の最後の測定を最大1回/分で書き込みます。グラフは1分あたり1点、直近30分（境界を含め最大31点）だけです。通常時は30分より古い行を物理削除します。測定時刻はAndroidの実時計を使いますが、手動設定や時刻同期で時計が巻き戻って既存行が一時的に「未来」に見えても、その正常行を即座に物理削除しません。現在の30分窓からは非表示にし、時刻が追いつけば再び読み出せるようにしつつ、DB全体は最大31行へ制限します。
...
- Android の deep sleep 中でも5/15/60秒の継続計測を維持するため、Foreground Service の計測中だけ `PARTIAL_WAKE_LOCK` を保持します。WakeLockは10分の安全タイムアウト付きで、計測継続中は5分ごとに更新し、通知から計測を停止すると即座に解放します。
...
- DNS-SD検索は「＋ 端末」のダイアログを開いている間だけ動作します。探索レコードは旧/新NSD API、解決待ち、遅延再試行を合算して最大32サービスに制限し、消失端末の墓標と再試行Runnableにも上限・期限があります。
...
- 通常時の履歴は直近30分・最大31点です。時計巻き戻しで一時的に将来扱いになった正常行は破壊防止のため最大31行の範囲で一時保持しますが、現在窓の表示・共有には含めません。Android バックアップ／端末移行からも除外します。
...
この設定時だけ `dist/BatteryRelay-1.3.1.apk` を生成します。release鍵を設定していない場合は、開発専用の `.dev-signing.jks` を使って `dist/BatteryRelay-1.3.1-dev.apk` を生成し、配布版と明確に区別します。既存の開発キーストアとの互換性のため標準aliasは従来どおり `battery-relay` です。Gradleのrelease assemble/bundle/packageもrelease署名環境変数が無ければ失敗させるため、別PCで勝手に新しい配布鍵を作って既存APKを更新不能にする事故を防ぎます。
